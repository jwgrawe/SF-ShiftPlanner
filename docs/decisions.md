...
| D69 | R4 | **The wall board has a light theme.** `/display?tema=lys` swaps a token set for a bright room; the layout, zone colours and fit-to-screen behaviour are identical. The wall PC just keeps the URL it was opened with; a theme toggle appears only in preview (`?date`/`?time`), never on the live board. Default stays dark. |
| D70 | R4 | **«Diverse info» is multi-line** (a textarea; line breaks are preserved on the board) and its editor is tinted in the accent family — deliberately not the warning amber used by the findings list directly below it. |
