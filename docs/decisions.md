...
| D72 | R4 | **Board rows are readable across the width**: employee names are bold, every row carries a hairline rule, and a dotted leader spans the gap from the name to its rotation chip (only on rows that have one) so the eye can follow a person to their next placement on a large screen. |
